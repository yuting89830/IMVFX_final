...
def readkeypointsfile_yml(myfile):
	...
	find_data = [x.startswith("data") or x=='' for x in leftovers]
	print(find_data)
	# print(leftovers)
	data_ind = np.where(np.array(find_data))[0]

	leftovers1 = leftovers[data_ind[0]:data_ind[1]]
	leftovers2 = leftovers[data_ind[2]:]
	print('1', leftovers1)
	# print('2', leftovers2)
	...
	datastr= reduce(lambda x, y: x + y, leftovers1)
	datastr = datastr.replace("\n", "")
	bigstring = datastr[7:len(datastr)-2]
	coords1 = [float(x.strip()) for x in bigstring.split(',')]
	
	datastr= reduce(lambda x, y: x + y, leftovers2)
	datastr = datastr.replace("\n", "")
	bigstring = datastr[7:len(datastr)-2]
	coords2 = [float(x.strip()) for x in bigstring.split(',')]
...
def readkeypointsfile_json(myfile):
	...
	l_handpts = []
	for p in people:
		posepts.append(list(p['pose_keypoints_2d']))
		# facepts.append(list(p['face_keypoints_2d']))
		r_handpts.append(list(p['hand_right_keypoints_2d']))
		l_handpts.append(list(p['hand_left_keypoints_2d']))

	# print('pose1:', posepts[0])
	# print('facepts1:', facepts[0])
	# print('r_handpts1:', r_handpts[0])
	# print('l_handpts1:', l_handpts[0])

	# print('pose2:', posepts[1])
	# print('facepts2:', facepts[1])
	# print('r_handpts2:', r_handpts[1])
	# print('l_handpts2:', l_handpts[1])

	return posepts, r_handpts, l_handpts
